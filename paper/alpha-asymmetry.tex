...
{\footnotesize
  ...
  T.\ Israfilov: \href{mailto:tofiq@dissensus.ai}{tofiq@dissensus.ai}%
}\\[0.3em]
...
($\hat{\gamma}_1 = 1.75$, 95\% CI $[1.18, 2.16]$); the signed tail signal skews
\emph{negative} rather than positive ($-1.48$, CI $[-3.10, 0.54]$), and momentum,
mean-reversion, and correlation signals have skewness intervals that include zero.
...
(intercept $t = -0.84$ on in-position weeks). It does, however, carry a
significant \emph{negative} loading on time-series momentum
($\beta = -0.82$, 95\% CI $[-1.44, -0.20]$ on in-position weeks), absent before
correction, indicating a substantial short-momentum exposure alongside whatever
asymmetry signal the rule captures. Transaction costs make an already negative gross
result monotonically worse; no break-even cost exists, since the strategy does
not break even at zero cost.
Data-snooping corrections complete the null: White's Reality Check ($p = 0.30$)
and Hansen's SPA ($p = 0.25$) find no strategy in a 12-candidate universe that
outperforms the zero-return benchmark, and neither does a thirteenth candidate
...
Our empirical contribution is threefold. First, we provide a systematic characterization of distributional properties across five distinct alpha types in EUR/JPY forex data. Second, the corrected strategy loses money before frictions, and walk-forward selection leaves it nearly dormant. Third, unchanged cross-market transfers range from a 15.44\% loss to a 14.20\% gain, without market-specific costs or inference; these are exploratory instability checks, not evidence of generalization.
...
On the block-bootstrap yardstick only \emph{coverage} alpha survives: its interval $[1.18, 2.16]$ excludes zero decisively. Tail, fast, pricing and hedge alpha intervals all include zero.

\textbf{What survives.} The asymmetry inventory that motivates the rest of the paper is therefore modest: one robustly right-skewed volatility-expansion signal, one negatively-skewed-but-fragile tail signal, and three signals whose skewness intervals include zero. This is consistent with the broader stylized facts on asset-price distributions \citep{cont2001empirical, mandelbrot1963variation} while falling well short of the pervasive ``alpha asymmetry'' a trading strategy would want.
...
\textbf{Little Asymmetry to Exploit.} The dependence-robust inventory leaves one robustly skewed signal (coverage) and one fragile negative point estimate (tail). The trading rule instead uses fast and pricing skewness, whose full-sample intervals include zero. Its 15 in-sample episodes do not establish profitable predictability.
...
\textbf{It Is Partly Trading Momentum.} The corrected factor regression carries a statistically significant \emph{negative} loading on time-series momentum ($\beta_2 = -0.82$, 95\% CI $[-1.44, -0.20]$, $p = 0.0091$ on in-position weeks; $-0.046$, $p = 0.038$ on the full sample), where under the published specification no loading approached significance. A rule that sells strength and buys weakness is taking a documented factor exposure alongside whatever asymmetry signal it captures. Momentum rose over the exposed weeks and the strategy was short it, which accounts for roughly a quarter of the realized loss; it explains 12\% of the variation in in-position returns, leaving most of it unexplained. The negative result is therefore not only that asymmetry failed to pay, but that part of what was being tested was a momentum exposure rather than asymmetry.

\textbf{Non-Generalizability.} Cross-market transfer yields erratic results: the identical rules lose 13.32\% on GBP/USD, gain 14.20\% on SPY while trailing buy-and-hold by roughly 280 percentage points, and lose 15.44\% on GLD. The tail signal's negative point estimate also reverses sign in every other market tested. Without asset-specific costs or inference, nothing here establishes generalization.
...
\midrule
Intercept ($\alpha$) & $-$0.00012 & $-$0.52 & $-$0.00131 & $-$0.67 \\
Carry ($\beta_1$) & 0.0180 & 0.60 & 0.348 & 1.49 \\
Momentum ($\beta_2$) & $-$0.0462 & $-$2.08$^{*}$ & $-$0.823 & $-$2.61$^{**}$ \\
Dollar ($\beta_3$) & 0.0148 & 0.67 & 0.155 & 0.98 \\
\midrule
$R^2$ & \multicolumn{2}{c}{0.011} & \multicolumn{2}{c}{0.203} \\
$F$-statistic & \multicolumn{2}{c}{2.12} & \multicolumn{2}{c}{7.04} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Full-sample column uses Newey-West HAC standard errors with 4 lags. \textbf{The in-position column uses standard errors clustered by holding episode} (55 weeks in 15 clusters), because those weeks are non-contiguous---they are drawn from separate episodes spread across a decade---and a lag-based correction presumes consecutive observations. Table~\ref{tab:sevariants} reports the momentum coefficient under all three conventions. $^{*}$ $p < 0.05$, $^{**}$ $p < 0.01$ (unadjusted). The full sample includes the flat weeks in which the dependent variable is structurally zero; the in-position sample of 55 weeks is the effective exposure sample and the economically interpretable one. The intercept is not significant in either specification. Momentum is the only factor that loads significantly, and it loads \emph{negatively} in both: $p = 0.038$ on the full sample and $p = 0.00019$ on in-position weeks. Under the earlier specification the same coefficient was $-$0.007 ($t = -0.37$, $p = 0.71$) on the full sample and $-$0.247 ($t = -0.51$, $p = 0.61$) in position; the change is a consequence of correcting the holding rule and the execution lag, not of altering the regression. Generated by \texttt{analysis/full\_pipeline.py}; an earlier version of this table reported a marginally significant intercept of 21 bps weekly (10.9\% annualized) with $F = 20.8$, which could not be reproduced and was internally inconsistent with its own $R^2$ and with the 3.60\% cumulative backtest return. Those figures describe the state of the paper at that time; the corresponding values here are an intercept of $-$1.2 bps weekly ($-$0.00012 in weekly decimal return) and $F = 2.12$, against a cumulative backtest return of $-6.64\%$.}
\end{table}

\textbf{No Evidence of Positive Residual Alpha.} The intercept is negative and statistically indistinguishable from zero in both specifications ($t = -0.52$ full sample; $t = -0.84$ on in-position weeks). This is evidence against a positive residual-alpha claim, not proof that the population intercept is exactly zero.

\textbf{A Significant Negative Momentum Loading.} The correction changes this exercise qualitatively, and not in the strategy's favour. Under the published specification no factor loading approached significance: momentum entered at $\beta_2 = -0.007$ ($t = -0.37$, $p = 0.71$) on the full sample and $-0.247$ ($t = -0.51$, $p = 0.61$) on in-position weeks, and the paper concluded that the regression had no power to say anything. With the holding rule and the execution lag corrected, momentum becomes the one factor that loads significantly, and it loads negatively: $\beta_2 = -0.046$ ($t = -2.08$, $p = 0.038$) on the full sample and $\beta_2 = -0.823$ ($t = -2.61$, $p = 0.0091$, clustered by holding episode) on the 55 in-position weeks, where $R^2$ is 0.20 against 0.07 on the published 25-week sample. Carry and the dollar factor remain insignificant, and the intercept remains indistinguishable from zero.

The in-position estimate is the economically meaningful one: the full-sample coefficient is mechanically attenuated because the dependent variable is exactly zero in 449 of 504 weeks, which shrinks any loading toward zero without saying anything about the exposure the strategy actually carries when it is in the market. Read on the exposure sample, $\beta_2 = -0.82$ (95\% CI $[-1.44, -0.20]$) says that while invested the strategy carries a large negative exposure to twelve-week time-series momentum on the pair it trades. The point estimate is close to one-for-one, but the interval spans roughly half to full unit exposure, and the magnitude should not be pressed further than that. This is consistent with a mechanical consequence of the rule, though we do not test the mechanism directly: the short leg fires when pricing alpha is high, which is to say after the price has run above its sixty-day average, so the strategy systematically sells strength and buys weakness. Momentum alone accounts for 12\% of the variance of in-position returns; the three factors together account for 20\%.

This matters for the paper's central question. A strategy carrying a large short-momentum exposure is, to that extent, not harvesting distributional asymmetry but taking a documented factor exposure. Over the 55 exposed weeks the twelve-week momentum factor \emph{gained} 1.93\%, and the strategy was short it: at $\beta_2 = -0.82$ that exposure accounts for roughly $-1.6$ percentage points of the $-6.64\%$ loss, about a quarter of it. (The twenty-week momentum benchmark of Table~\ref{tab:backtest} lost 18.92\% over the full sample, but that is a different series from the twelve-week regressor and is not the relevant comparison here.) This sharpens the null rather than softening it: there is no residual alpha to explain, and a measurable part of the return variation that does exist is explained by an exposure the strategy never set out to take.

\textbf{How Far to Press It.} The in-position weeks are non-contiguous: 55 weeks drawn from 15 separate holding episodes spread across a decade. A lag-based correction such as Newey-West presumes that adjacent rows are adjacent in time, which here they are not---consecutive rows in the stacked sample can be years apart. We therefore report standard errors clustered by holding episode as the primary specification, with the lag-based and heteroskedasticity-robust alternatives beside them.

\begin{table}[H]
\centering
\caption{Momentum Loading Under Three Standard-Error Conventions (In-Position Weeks)}
...
Newey-West HAC, 4 lags & $-$0.823 & 0.221 & $-$3.73 & 0.00019 \\
HC3 heteroskedasticity-robust & $-$0.823 & 0.380 & $-$2.17 & 0.0304 \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: 55 in-position weeks in 15 clusters. The point estimate is identical throughout; only the standard error differs. Clustering is primary because it is the only one of the three that respects the episodic structure of the sample.}
\end{table}

Under the primary specification the loading is $\hat{\beta}_2 = -0.823$ with $t = -2.61$ and $p = 0.0091$ (95\% CI $[-1.44, -0.20]$). Four qualifications bound the finding.

First, on multiple testing: this paper applies Bonferroni correction elsewhere, and at family size six---three factors in two specifications---the threshold would be $0.0083$. \textbf{At $p = 0.0091$ the momentum loading falls just short of that threshold.} It is significant at the 5\% level under every standard-error convention we tried, and it does not clear the paper's own multiple-testing bar. An earlier draft of this revision reported $p = 0.00019$ from the lag-based errors and described it as clearing that bar comfortably; that claim depended on a correction inappropriate to a non-contiguous sample, and we withdraw it.

Second, Mom is a twelve-week time-series momentum rule on EUR/JPY itself, not the published cross-sectional currency momentum factor of \citet{menkhoff2012currency}. A strong loading of a single-pair strategy on a single-pair momentum rule is less surprising, and less transferable, than a loading on a genuine cross-sectional factor would be.

Third, 15 clusters is a small number for cluster-robust inference, which is asymptotic in the number of clusters rather than the number of observations. The $p$-value should be read as indicative rather than exact.

Fourth, momentum leaves roughly seven-eighths of the variation in in-position returns unexplained. The exposure is economically material without being the whole story. The loading is a robust description of this sample and a hypothesis about the strategy's character, not a structural estimate.
...
A five-week separation is aggressive for weekly data, where persistence is far shorter than in the daily series declustering was designed for, and we report its consequences rather than assert its harmlessness. Varying the separation from one to five weeks changes the number of cluster maxima from 22 to 20 and the shape estimate from $-0.209$ to $-0.248$; the three- and five-week settings are identical, and every interval spans roughly $[-1.5, 0.2]$ (Table~\ref{tab:decluster}).
...
The choice is therefore immaterial: the loosest setting we tried recovers two additional cluster maxima and moves the shape estimate by 0.04, well inside an interval that spans more than 1.7. The imprecision of the tail estimate is a consequence of having 26 exceedances in ten years, not of how they are declustered. We estimate the extremal index $\theta = 0.83$ via the Ferro--Segers intervals estimator \citep{ferro2003inference}, with a 95\% confidence interval of $[0.60, 1.00]$ obtained by bootstrap resampling of the inter-exceedance times themselves ($B = 2000$)---a scheme that preserves the gap distribution the estimator is built on and, unlike a naive index bootstrap, produces an interval containing its own point estimate.
...
\textbf{Tail Characterization.} The estimated shape parameter $\hat{\xi} = -0.25$ (95\% CI: $[-1.49, 0.27]$) is imprecise. The interval admits bounded, exponential-type, and moderately Pareto-type behavior; the data therefore do not support a strong tail-classification claim. The Kolmogorov-Smirnov test does not reject the fitted GPD ($p = 0.97$), but with only 20 cluster maxima it has limited diagnostic power.
...
\textbf{Why These Statistics Are Insensitive to the Sizing Specification.} Both test statistics are numerically identical under the weekly and frozen-notional sizing specifications, and the $p$-values differ by 0.001 and 0.000 respectively. This is not a transcription error. Both tests take a maximum across the thirteen-candidate universe, and that maximum is attained by the seeded random candidate, whose returns do not depend on the asymmetry strategy's sizing at all. The asymmetry strategy enters the universe as one candidate among thirteen and is nowhere near the maximum, so a change to it moves the test statistic not at all and reaches the $p$-value only through the bootstrap covariance between candidates. A reader should expect these two rows to be stable across any change confined to the asymmetry rule.
...
\midrule
White's Reality Check & 0.015 & 0.30 & Not significant \\
...
\begin{enumerate}
    \item Only coverage alpha has a dependence-robust skewness interval that excludes zero; the other four intervals include zero
    ...
    \item While invested, the corrected strategy loads significantly and negatively on time-series momentum ($\beta_2 = -0.82$, 95\% CI $[-1.44, -0.20]$), a loading absent before correction. Momentum rose over the exposed weeks and the strategy was short it, accounting for roughly a quarter of the realized loss, so part of that loss reflects a known factor exposure rather than a failure specific to asymmetry
...
\paragraph{AI Assistance.} Claude (Anthropic) assisted both authors, in distinct phases, each author directing and remaining answerable for the assistance used in their own. M.\ Farzulla directed its use in the original version for manuscript drafting, code review, and statistical exposition. T.\ Israfilov directed its use for the corrections in the present version: reading the published replication code against the published specification, implementing the corrections, regenerating the analysis, and drafting the revised text.

Because implementing corrections is a broader role than assisting with prose, the work was structured so that it can be checked rather than trusted. Every change is a separate commit recording its rationale; the derivation of each correction from the published version is documented in the repository's correction changelog; each reported figure is machine-checked against the pipeline's own output; the restored provenance footnotes are machine-checked for verbatim preservation; and the analysis is covered by deterministic tests required to pass at every step. Two arithmetic identities---the factor intercept against the strategy's own mean weekly return, and the volatility-regime returns compounding to the full-sample return---are verified after every rerun.

That apparatus caught errors that review by reading did not, including a passage in the factor-attribution section whose stated reasoning ran opposite to its own conclusion. We regard it as part of the contribution rather than as housekeeping: it is what allows a reader to check these corrections instead of taking them on trust, and it is reusable by anyone auditing a quantitative result produced the same way. All research design, and all responsibility for the content of this paper, remain the authors'.

\paragraph{Author Contributions.} \textbf{M.\ Farzulla:} original research question and design; alpha signal construction; strategy specification; statistical methodology; original analysis code; manuscript. \textbf{T.\ Israfilov:} audit of the published version against its replication code; identification of the specification mismatches between the two; decisions on how each mismatch was resolved, including which were implementation defects and which required a specification choice; the correction record and its verification. T.\ Israfilov did not author the original analysis code. Both authors reviewed and approved the corrected manuscript; M.\ Farzulla retains responsibility for the original research design and its interpretation.
