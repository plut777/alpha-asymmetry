...
(intercept $t = -0.84$ on in-position weeks). Conditioning on the weeks it is
...
Our empirical contribution is threefold, and we state it narrowly. First, we provide a systematic characterization of distributional properties across five distinct alpha types in EUR/JPY forex data, of which one---the volatility-expansion signal---survives dependence-robust inference under the paper's primary construction. Second, we report a corrected analysis of the trading rule: it loses money before frictions, walk-forward selection leaves it nearly dormant, and no candidate in a twelve-strategy universe beats a zero-return benchmark. Third, we document the specification sensitivity of those results---to the weekly aggregation of the tail signal, and to the execution timing---at magnitudes this sample cannot resolve. We do not claim a new empirical finding about factor exposure; the momentum loading reported in Section~\ref{sec:factors} is a property of the entry rules, not a discovery about the market.
