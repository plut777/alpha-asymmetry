...
skewness-threshold strategy loses 0.73\% cumulative gross over the decade (15
...
(intercept $t = -0.45$ on in-position weeks). Conditioning on the weeks it is
...
Data-snooping corrections complete the null: neither White's Reality Check ($p = 0.30$) nor Hansen's SPA ($p = 0.58$) rejects the null of no superior performance against
...
set. The weekly-return GPD shape parameter is imprecisely estimated ($\xi = -0.25$, 95\% CI $[-1.49, 0.27]$---an interval wide enough, and containing zero, that neither the sign nor a bounded-versus-heavy-tail reading is established), and tail clustering is weak ($\theta = 0.83$, 95\% CI $[0.60, 1.00]$). We conclude
...
\textbf{Data Frequency and Aggregation.} \textbf{Executable Sample.} A position is realised from the first trading-session open after its signal, so the final week of the sample has no subsequent open and no realisable return. The analysis panel retains all 504 weeks, because the alpha signals are defined for every one of them, while performance statistics are computed over the 503 weeks that carry a realisable return. A week without an executable price is treated as missing rather than as a zero return: recording it as zero would place an observation that does not exist into the mean, the volatility, the bootstrap and the factor regression as though it had been observed.
...
Execution price convention & first session open after signal & price & Timing convention \\
Weeks with a realisable return & 503 & weeks & Sample \\
...
\midrule
Asymmetry & $-$0.73\% & 4.42\% & 0.005 & 0.003 & $-$10.64\% & 15 / 61 \\
Momentum (20w) & $-$23.48\% & 8.51\% & $-$0.282 & $-$0.393 & $-$34.77\% & 54 / 107 \\
Mean Rev.\ (2.0$\sigma$) & $-$1.27\% & 3.48\% & $-$0.021 & $-$0.010 & $-$7.63\% & 27 / 54 \\
Buy \& Hold & 34.63\% & 8.77\% & 0.394 & 0.608 & $-$15.40\% & 1 / 1 \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\textbf{Risk-Adjusted Performance.} The corrected asymmetry strategy loses 0.73\% gross, with a Sharpe ratio of $+0.005$ that is indistinguishable from zero, versus a 34.63\% buy-and-hold gain. It is invested for 55 of 504 realized-return weeks. Its low 4.42\% annualized volatility still largely reflects inactivity rather than skill. The 20-week momentum benchmark also loses money, while the simple mean-reversion and buy-and-hold benchmarks are positive.

\textbf{Drawdown Analysis.} The asymmetry strategy's maximum drawdown is $-10.64\%$, against buy-and-hold's $-15.40\%$. Given that it is exposed in only 55 of 504 weeks, losing two-thirds of what a permanently invested position lost is not evidence of risk control. Mean reversion remains more contained at $-7.63\%$.
...
\textbf{Statistical Significance of Returns.} Stationary-bootstrap inference (expected block length 4 weeks, $B = 2000$) gives an annualized return of $-0.08\%$ with a Sharpe interval of $[-0.51, 0.49]$. The sample estimate is slightly negative and the interval is far too wide to establish a nonzero population mean. Failure to reject zero is not proof that the true effect is exactly absent.
...
\midrule
EUR/JPY & $-$1.48 & 0.01 & $-$0.17 & $-$0.73\% & 33.53\% \\
GBP/USD & 0.82 & 0.01 & $-$0.14 & $-$14.45\% & $-$7.63\% \\
SPY & 0.94 & $-$0.21 & $-$1.09 & 14.71\% & 294.30\% \\
GLD & 0.69 & $-$0.00 & $-$0.19 & $-$13.03\% & 200.97\% \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\midrule
Pre-COVID (2016--2019) & 208 & -3.40 & -0.05 & 0.12 & $-$7.08\% \\
COVID Shock (2020) & 52 & 5.02 & 0.09 & -0.07 & 2.77\% \\
Post-COVID (2021--2025) & 244 & -0.73 & 0.05 & -0.38 & 3.96\% \\
Low VIX (VIX $<$ 20) & 342 & -3.25 & 0.04 & -0.20 & $-$1.69\% \\
High VIX (VIX $\geq$ 20) & 162 & 0.62 & -0.06 & -0.10 & 0.98\% \\
Rate Hike Regime (2022--2025) & 191 & -0.71 & -0.13 & -0.53 & 7.21\% \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\midrule
0.50 & 25 & 89 & $-$10.55\% & $-$0.19 & 49.4\% \\
0.75 & 15 & 55 & $-$0.73\% & 0.00 & 49.1\% \\
1.00 & 4 & 16 & $-$1.25\% & $-$0.06 & 50.0\% \\
...
\midrule
Friday close & $-$6.64\% & $-$1.21 bps & $-$0.154 & $-$12.56\% & $-$7.02\% \\
Monday open (reported baseline) & $-$0.73\% & $+$0.04 bps & $+$0.005 & $-$10.64\% & $-$1.14\% \\
...
\midrule
Published hybrid (reported baseline) & $-$0.73\% & $-$1.14\% & 0.005 & $-$10.64\% & 15 & 55 & 0.4 \\
Pure-fast (both legs, fast alpha) & $-$3.48\% & $-$3.97\% & $-$0.067 & $-$12.05\% & 16 & 44 & $-$6.1 \\
Pure-pricing (both legs, pricing alpha) & 5.57\% & 5.21\% & 0.178 & $-$11.97\% & 14 & 51 & 11.8 \\
Equal-threshold hybrid & $-$1.65\% & $-$1.96\% & $-$0.029 & $-$10.83\% & 12 & 43 & $-$2.4 \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
Realised performance varies materially across the four pre-specified entry rules, and the variation includes a change of sign. The published hybrid loses 0.73\% gross; pure-fast loses 3.48\%; the equal-threshold hybrid loses 1.65\%; and \textbf{pure-pricing gains 5.57\% gross, 5.21\% net of the widest cost tier}. The spread is 9.04 percentage points, against a published-hybrid loss of 0.73\%---an order of magnitude larger than the result it surrounds.

\textbf{What the positive variant does and does not establish.} Pure-pricing is a pre-specified variant, not a rule found by searching. Its return is reported because the pre-registration required all three symmetrizations to be disclosed whatever they produced, and that requirement was committed before the execution convention was decided and before any of these figures existed. It is \emph{not} offered as a preferred strategy, and no comparative inference among the four was pre-specified or run, so nothing here establishes that pure-pricing has positive expected alpha. What it establishes is narrower: \textbf{the sample does not support a stable performance conclusion across the pre-specified entry-rule constructions.}

The claim this paper made in an earlier draft---that the economic failure is robust to how the entry asymmetry is resolved---is no longer supportable, and we withdraw it rather than qualify it. Four defensible readings of one rule, differing only in a design choice the original specification never argued for, produce outcomes spanning from a 3.48\% loss to a 5.57\% gain.

Exposure does not explain the spread. Pure-fast and pure-pricing hold positions in fewer weeks than the published rule---44 and 51 against 55---and the ordering is the same per exposed week as it is cumulatively: $-$6.1 and $+$11.8 basis points against the published rule's $+$0.4. A rule that is out of the market more often is not thereby made to look better here.

Equalising the confirmation threshold worsens realised performance under both execution conventions, by 1.22 percentage points under Friday close and 0.92 under Monday open. This is one comparison and is not evidence that the published threshold was tuned; its direction is merely consistent with what a specification-search concern would predict.
...
\midrule
Zero Cost (Baseline) & 0.0 & $-$0.73\% & 0.005 \\
Prime Brokerage & 0.3 & $-$0.79\% & 0.003 \\
Institutional & 0.7 & $-$0.87\% & 0.001 \\
Retail (Tight) & 1.3 & $-$0.99\% & $-$0.001 \\
Retail (Wide) & 2.0 & $-$1.14\% & $-$0.005 \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\textbf{Cost Interpretation.} Execution costs make an already negative result more negative, monotonically and by little: gross $-0.73\%$ becomes $-1.14\%$ at two pips round-trip, a drag of 0.41 percentage points. Under the previous execution convention the gross loss dwarfed that drag; it no longer does, and the cost tier is now of the same order as the gross result itself.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
& \multicolumn{2}{c}{Full sample ($n = 503$)} & \multicolumn{2}{c}{In-position weeks ($n = 55$)} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Variable & Coefficient & $t$-stat & Coefficient & $t$-stat \\
\midrule
Intercept ($\alpha$) & 0.00000 & 0.01 & $-$0.00095 & $-$0.45 \\
Carry ($\beta_1$) & 0.0159 & 0.63 & 0.218 & 1.11 \\
Momentum ($\beta_2$) & $-$0.0375 & $-$1.71 & $-$0.652 & $-$2.33$^{*}$ \\
Dollar ($\beta_3$) & 0.0607 & 1.59 & 0.622 & 2.64$^{*}$ \\
$R^2$ & \multicolumn{2}{c}{0.013} & \multicolumn{2}{c}{0.170} \\
$F$-statistic & \multicolumn{2}{c}{1.47} & \multicolumn{2}{c}{3.47} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: Full-sample column uses Newey-West HAC standard errors with 4 lags. \textbf{The in-position column uses CR2 standard errors clustered by holding episode} (55 weeks in 15 clusters) with Bell--McCaffrey degrees of freedom, because those weeks are non-contiguous---they are drawn from separate episodes spread across a decade---and a lag-based correction presumes consecutive observations. The reported $p$-values for momentum come from a restricted wild cluster bootstrap; see Table~\ref{tab:sevariants}. $^{*}$ $p < 0.05$ (unadjusted). The full sample includes the flat weeks in which the dependent variable is structurally zero; the in-position sample of 55 weeks is the effective exposure sample and the economically interpretable one. The intercept is not significant in either specification. Momentum is negative in both specifications but is no longer the most strongly loading factor. Compared like for like, on CR2 standard errors from the same estimator and clustering, the dollar proxy carries the larger $t$-statistic on in-position weeks ($+2.64$, CR2 $p = 0.034$) than momentum does ($-2.33$, CR2 $p = 0.042$). The reported momentum $p$-value of 0.0506 comes from the restricted wild cluster bootstrap and is not comparable with either CR2 figure; the bootstrap was computed for momentum alone. On the full sample momentum reaches only $p = 0.087$. The sample-selection problem described below applies to every coefficient in the in-position regression alike, so no coefficient in that column is offered as an identified exposure. Under the earlier specification the same coefficient was $-$0.007 ($t = -0.37$, $p = 0.71$) on the full sample and $-$0.247 ($t = -0.51$, $p = 0.61$) in position; the change is a consequence of correcting the holding rule and the execution lag, not of altering the regression. Generated by \texttt{analysis/full\_pipeline.py}; an earlier version of this table reported a marginally significant intercept of 21 bps weekly (10.9\% annualized) with $F = 20.8$, which could not be reproduced and was internally inconsistent with its own $R^2$ and with the 3.60\% cumulative backtest return. Those figures describe the state of the paper at that time; the corresponding values here are an intercept of $+0.04$ bps weekly ($+0.00000351$ in weekly decimal return) and $F = 1.47$, against a cumulative backtest return of $-0.73\%$.}
\end{table}

\textbf{No Evidence of Positive Residual Alpha.} The intercept is negative and statistically indistinguishable from zero in both specifications ($t = 0.01$ full sample; $t = -0.45$ on in-position weeks). This is evidence against a positive residual-alpha claim, not proof that the population intercept is exactly zero.
...
The negative loading on in-position weeks ($\beta_2 = -0.652$, CR2 interval $[-1.27, -0.03]$, wild cluster bootstrap $p = 0.0506$) should therefore be read as \textbf{a description of what the entry rules mechanically produce, not as an estimate of a factor exposure the strategy carries independently of them}. A rule that sells strength and buys weakness will load negatively on momentum during the weeks it is active whether or not any factor relationship exists in the underlying returns. We report the coefficient because it characterises the rule. We do not interpret it as explaining the strategy's losses, and we do not present it as an empirical finding about the market.

The full-sample regression is reported alongside as an economically relevant description of the complete strategy return stream: it is what an investor holding the rule would have experienced, flat weeks included. There the loading is $\beta_2 = -0.038$ ($p = 0.087$), attenuated by the 448 weeks in which the dependent variable is structurally zero, and it does not meet the paper's Bonferroni-adjusted threshold. \textbf{It does not solve the identification problem either.} Including the inactive weeks changes what is being described; it does not make the active weeks independent of the entry rule that selected them. The intercept is indistinguishable from zero in both specifications, and neither supports a claim about residual alpha.

\textbf{Inference Was Tightened Three Times, and That Was Not the Binding Problem.} The estimated significance of this loading moved substantially as the inferential treatment improved: $p = 0.00019$ under the Newey-West treatment first applied, $p = 0.0091$ once errors were clustered by holding episode, and, under CR2 with a restricted wild cluster bootstrap appropriate to fifteen clusters, a pair of figures that no longer agree: the CR2 interval $[-1.27, -0.03]$ excludes zero at $p = 0.042$ while the bootstrap returns $p = 0.0506$. The 5\% line now falls between the two methods. Each step was a correction, and the first two were withdrawn as inapplicable rather than merely superseded. Fifteen clusters is few, and ordinary cluster-robust errors are asymptotic in the number of clusters rather than the number of observations, so they over-reject when clusters are scarce; the reported figures use the bias-reduced CR2 estimator of \citet{bell2002bias} with Satterthwaite degrees of freedom---10.2 here rather than the naive 14---and take the $p$-value from a restricted wild cluster bootstrap-$t$ with Rademacher weights \citep{cameron2008bootstrap}. Even these are approximations at fifteen clusters, and the $p$-value should be read as indicative rather than exact. Table~\ref{tab:sevariants} reports all of it.
...
\midrule
Wild cluster bootstrap-$t$ & $-$0.652 & --- & $-$2.33 & \textbf{0.0506} & \textbf{Primary} \\
CR2, Bell--McCaffrey dof $= 10.2$ & $-$0.652 & 0.280 & $-$2.33 & 0.042 & Standard error and interval \\
HC3 & $-$0.652 & 0.346 & $-$1.88 & 0.060 & Robustness \\
\midrule
Newey-West HAC, 4 lags & $-$0.652 & 0.251 & $-$2.60 & 0.0094 & \emph{Withdrawn} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
\parbox{\linewidth}{\footnotesize Note: 55 in-position weeks in 15 holding episodes. The point estimate is identical throughout; only the inference differs. Ordinary cluster-robust errors are asymptotic in the number of clusters, and fifteen is few, so CR2 supplies the standard error and the 95\% interval $[-1.64, -0.37]$, and a restricted wild cluster bootstrap-$t$ with Rademacher weights ($B = 9{,}999$, null imposed) supplies the primary $p$-value. \textbf{The Newey-West row is not an alternative specification.} It is the figure published in an earlier draft of this revision, shown only so the withdrawn number is identifiable: a lag-based autocorrelation correction presumes consecutive observations, which these are not, and it is inapplicable here.}
\end{table}
...
\textbf{White's Reality Check.} Following \citet{white2000reality}, we test the twelve-strategy universe against a zero-return benchmark. The Reality Check does not reject the null of no superior strategy ($RC = 0.015$, $p = 0.30$). Including the random sequence as a thirteenth candidate gives $RC = 0.025$, $p = 0.06$. The direction is worth noting because it is the opposite of what candidate-dilution intuition suggests: the random sequence has the highest realized sample mean in the universe, so removing it lowers the observed statistic more than it lowers the bootstrap distribution, and the $p$-value \emph{rises}. Neither universe rejects.

\textbf{Hansen's SPA Test.} The Superior Predictive Ability test \citep{hansen2005test} agrees ($SPA = 1.43$, $p = 0.58$ on the twelve-strategy universe; $p = 0.22$ including the random sequence): no candidate demonstrates superior predictive ability over the zero-return benchmark at conventional levels.

\textbf{Why These Statistics Are Insensitive to the Sizing Specification.} Both test statistics are numerically identical under the weekly and frozen-notional sizing specifications, and the $p$-values differ by 0.002 and 0.002 respectively. This is not a transcription error, and the reason is a property of the formal universe that we verify rather than assert. Of the twelve formal candidates, exactly one---the full asymmetry strategy---depends on the sizing convention at all; the other eleven are built with fixed unit sizing or are buy-and-hold. Both tests take a maximum across that universe, and the maximum is attained by the always-long candidate, whose returns do not depend on the asymmetry rule in any respect. The observed statistic therefore cannot move when the sizing changes. The $p$-value can, because the bootstrap distribution is computed across the whole universe and does include the one candidate that changed, which is why the shift is small rather than exactly zero. A reader should expect these two rows to be stable across any change confined to the asymmetry rule, and should not read that stability as evidence about the rule. The argmax, the identity of the two statistics and the two $p$-value differences are all recorded in \texttt{data\_snooping.sizing\_invariance} in the replication output.
...
Hansen's SPA & 1.43 & 0.58 & Not significant \\
\# Candidate Strategies & 12 & --- & --- \\
\midrule
\multicolumn{4}{@{}l}{\emph{Including the random sequence as a thirteenth candidate:}} \\
White's Reality Check & 0.025 & 0.06 & Not significant \\
Hansen's SPA & 2.00 & 0.22 & Not significant \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\begin{enumerate}
    ...
    \item The corrected asymmetry strategy loses 0.73\% gross (15 holding episodes, Sharpe $+0.005$), with bootstrap intervals that include zero; walk-forward selection opens one episode in eight test years
    ...
    \item Conditioning on the weeks it is invested, the rule loads negatively on time-series momentum ($\beta_2 = -0.65$; CR2 $p = 0.042$ with an interval excluding zero, wild cluster bootstrap $p = 0.0506$---the methods disagree at the 5\% line). Because those weeks are selected by an entry condition defined on the same prices the factor is built from, we report this as a mechanical property of the rule and not as an empirical finding about factor exposure
