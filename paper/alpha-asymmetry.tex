...
(intercept $t = -0.87$ on in-position weeks). Conditioning on the weeks it is
...
set. The weekly-return GPD shape parameter is imprecisely estimated ($\xi = +0.12$, 95\% CI $[-1.67, 0.69]$), and tail clustering is moderate ($\theta = 0.70$, 95\% CI $[0.49, 1.00]$). We conclude
...
\midrule
Intercept ($\alpha$) & $-$0.00002 & $-$0.08 & $-$0.00200 & $-$0.87 \\
Carry ($\beta_1$) & 0.0166 & 0.65 & 0.248 & 1.52 \\
Momentum ($\beta_2$) & $-$0.0839 & $-$2.49$^{*}$ & $-$1.006 & $-$3.70$^{*}$ \\
Dollar ($\beta_3$) & 0.0634 & 1.68 & 0.519 & 2.08 \\
$R^2$ & \multicolumn{2}{c}{0.035} & \multicolumn{2}{c}{0.390} \\
$F$-statistic & \multicolumn{2}{c}{2.32} & \multicolumn{2}{c}{10.88} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\textbf{No Evidence of Positive Residual Alpha.} The intercept is negative and statistically indistinguishable from zero in both specifications ($t = -0.08$ full sample; $t = -0.87$ on in-position weeks). This is evidence against a positive residual-alpha claim, not proof that the population intercept is exactly zero.
...
The negative loading on in-position weeks ($\beta_2 = -1.006$, CR2 interval $[-1.64, -0.37]$, wild cluster bootstrap $p = 0.024$) should therefore be read as \textbf{a description of what the entry rules mechanically produce, not as an estimate of a factor exposure the strategy carries independently of them}. A rule that sells strength and buys weakness will load negatively on momentum during the weeks it is active whether or not any factor relationship exists in the underlying returns. We report the coefficient because it characterises the rule. We do not interpret it as explaining the strategy's losses, and we do not present it as an empirical finding about the market.

The full-sample regression is reported alongside as an economically relevant description of the complete strategy return stream: it is what an investor holding the rule would have experienced, flat weeks included. There the loading is $\beta_2 = -0.084$ ($p = 0.013$), attenuated by the 448 weeks in which the dependent variable is structurally zero, and it does not meet the paper's Bonferroni-adjusted threshold. \textbf{It does not solve the identification problem either.} Including the inactive weeks changes what is being described; it does not make the active weeks independent of the entry rule that selected them. The intercept is indistinguishable from zero in both specifications, and neither supports a claim about residual alpha.
...
\midrule
Wild cluster bootstrap-$t$ & $-$1.006 & --- & $-$3.70 & \textbf{0.024} & \textbf{Primary} \\
CR2, Bell--McCaffrey dof $= 7.6$ & $-$1.006 & 0.272 & $-$3.70 & 0.007 & Standard error and interval \\
HC3 & $-$1.006 & 0.254 & $-$3.97 & 0.000 & Robustness \\
\midrule
Newey-West HAC, 4 lags & $-$1.006 & 0.168 & $-$5.98 & 0.0000 & \emph{Withdrawn} \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\midrule
1 week & 21 & 0.239 & $[-1.50, 0.82]$ & 0.986 \\
2 weeks & 18 & 0.157 & $[-1.57, 0.65]$ & 0.985 \\
3 weeks & 18 & 0.157 & $[-1.57, 0.65]$ & 0.985 \\
5 weeks (reported) & 17 & 0.120 & $[-1.66, 0.66]$ & 0.880 \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
The shortest separation therefore recovers 4 additional cluster maxima and moves the shape estimate by 0.12, inside an interval spanning more than 2.3. The imprecision of the tail estimate is a consequence of having 26 exceedances in ten years, not of how they are declustered, and no conclusion in this section changes at any separation we tried. We estimate the extremal index $\theta = 0.70$ via the Ferro--Segers intervals estimator \citep{ferro2003inference}, with a 95\% confidence interval of $[0.49, 1.00]$ obtained by bootstrap resampling of the inter-exceedance times themselves ($B = 2000$)---a scheme that preserves the gap distribution the estimator is built on and, unlike a naive index bootstrap, produces an interval containing its own point estimate.
...
\midrule
Shape ($\xi$) & $+0.12$ & $[-1.67, 0.69]$ \\
Scale ($\sigma$) & 0.0065 & --- \\
Threshold (95th pctl) & 0.0250 & --- \\
Extremal index ($\theta$) & 0.70 & $[0.49, 1.00]$ \\
Raw exceedances & 26 & --- \\
Cluster maxima & 17 & --- \\
KS test $p$-value & 0.880 & Fail to reject \\
\bottomrule
\end{tabular}
\par\vspace{0.3em}
...
\textbf{Tail Characterization.} Table~\ref{tab:gpd} reports the fit. The estimated shape parameter $\hat{\xi} = +0.12$ (95\% CI: $[-1.67, 0.69]$) is imprecise. The interval admits bounded, exponential-type, and moderately Pareto-type behavior; the data therefore do not support a strong tail-classification claim. The Kolmogorov-Smirnov test does not reject the fitted GPD ($p = 0.97$), but with only 20 cluster maxima it has limited diagnostic power.

\textbf{Clustering Dynamics.} The extremal index $\theta = 0.70$ indicates moderate temporal clustering in tail events: the mean cluster size is $1/\theta \approx 1.4$, and the confidence interval still reaches 1.0 (no clustering). Exceedances arrive with mild GARCH-type persistence \citep{bollerslev1986generalized} at most. Clustering therefore does little to reduce the effective sample size for tail inference; the binding constraint is simply the small number of exceedances.
